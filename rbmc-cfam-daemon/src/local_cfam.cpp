...
void LocalCFAM::writeApiVersion(uint32_t version)
{
    auto rc = writeField(Field::apiVersion, version);
    if (rc != 0)
    {
        lg2::error("Failing writing API version {VERSION} in local CFAM",
                   "VERSION", version);
...
void LocalCFAM::writeFWVersion(uint32_t version)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing FW version {VERSION} in local CFAM",
                   "VERSION", lg2::hex, version);
        throw std::system_error(rc, std::generic_category());
    }
}

void LocalCFAM::writeBmcPosition(uint32_t position)
{
    auto rc = writeField(Field::bmcPosition, position);
    if (rc != 0)
    {
        lg2::error("Failing writing BMC position {POSITION} in local CFAM",
                   "VERSION", position);
        throw std::system_error(rc, std::generic_category());
    }
}
...
void LocalCFAM::writeProvisioned(bool isProvisioned)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing provisioned {PROVISIONED} in local CFAM",
...
void LocalCFAM::writeRedundancyEnabled(bool enabled)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing redundancy enabled {ENABLED} in local CFAM",
                   "ENABLED", enabled);
        throw std::system_error(rc, std::generic_category());
    }
}
...
void LocalCFAM::writeFailoversPaused(bool paused)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing failovers paused {PAUSED} in local CFAM",
                   "PAUSED", paused);
        throw std::system_error(rc, std::generic_category());
    }
}
...
void LocalCFAM::writeRole(LocalCFAM::Role role)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing role {ROLE} in local CFAM", "ROLE", role);
        throw std::system_error(rc, std::generic_category());
    }
}
...
void LocalCFAM::writeBMCState(LocalCFAM::BMCState state)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing BMC state {STATE} in local CFAM", "STATE",
                   state);
        throw std::system_error(rc, std::generic_category());
    }
}
...
void LocalCFAM::writeSiblingCommsOK(bool ok)
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing Sibling OK field {OK} in local CFAM", "OK",
                   ok);
        throw std::system_error(rc, std::generic_category());
    }
}
...
void LocalCFAM::incHeartbeat()
{
    ...
    if (rc != 0)
    {
        lg2::error("Failing writing BMC heartbeat {HEARTBEAT} in local CFAM",
